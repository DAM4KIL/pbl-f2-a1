...
function initAudio() {
  bossMusic = new Audio('mountainTrials.mp3.mp3');
  ...
  if (playPromise !== undefined) {
      playPromise.catch(error => {
          console.log("Autoplay blocked - music will start on first movement");
          // Fallback: Start music on first key press
          document.addEventListener('keydown', startMusicOnFirstInput, { once: true });
      });
  } else {
      isMusicPlaying = true;
  }
...
function playBossMusic() {
  if (!isBossMusicReady) return;

  const AudioContext = window.AudioContext || window.webkitAudioContext;
  const audioContext = new AudioContext();
  const source = audioContext.createBufferSource();
  
  source.buffer = bossMusic;
  source.loop = true;
...
let playerX = 50;
const groundLevel = 20; // Pixels from bottom
const airControl = 0.9; // Much more responsive air control
const airDrag = 0.98;
const attackSound = new Audio('blasterBoom.mp3');
...
// Constants
const moveSpeed = 5;
const jumpStrength = 13; // Higher initial burst
const gravity = 1.8; // Stronger pull downward
const airResistance = 0.96; // Smooths out jumps
const groundFriction = 0.85; // Stops sliding
const coyoteTime = 150; // 150ms grace period after leaving ground
const jumpBufferTime = 100; // 100ms input buffer
const maxFallSpeed = 25; // Limits crazy fast falls
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. Attempt autoplay (will fail silently in most browsers)
  const playPromise = bossMusic.play();
  
  // 3. Handle autoplay block (common in Chrome/Safari)
  if (playPromise !== undefined) {
      playPromise.catch(error => {
          // 4. Fallback: Show unobtrusive message
          console.log("Autoplay blocked - music will start after game interaction");
...
function updatePlayer() {
  ...
  playerX += moveX * deltaTime;
  playerX = Math.max(0, Math.min(gameContainer.offsetWidth - player.offsetWidth, playerX));

  // Jump handling
  const canJump = isGrounded || (currentTime - lastGroundedTime < coyoteTime);
...
function pickupMagicSquare() {
  gameEnded = true;
  stopBossMusic(); // Stop music when boss is defeated
...
function createProjectile(type, speed) {
  ...
  attackSound.currentTime = 0; // Rewind if already playing
    attackSound.play();
...
function gameOver() {
  gameEnded = true;
  clearInterval(currentAttackInterval);
  stopBossMusic(); // Stop music when game ends
...
function initGame() {
  ...
  updateMouthPosition();
  startPhase(1);
  updatePlayer(); // Start movement loop
  
  // Add visible "Click to start" prompt for audio
  ...
  audioPrompt.style.fontFamily = '"dogicabold", sans-serif';
  audioPrompt.style.textShadow = '0 0 5px red';
